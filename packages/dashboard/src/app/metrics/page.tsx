...
export default function MetricsPage() {
  ...
  // Apply theme from localStorage (synced with main app settings)
  useEffect(() => {
    const applyTheme = () => {
      try {
                  ...
                  <tbody>
                    {metrics.agents.map((agent) => (
                      <tr key={agent.name} className="border-b border-border/50 last:border-0 transition-colors hover:bg-bg-hover">
                        ...
                        <td className="py-3 px-4 font-mono text-sm text-accent-purple">{agent.messagesReceived.toLocaleString()}</td>
        ...
        {memoryMetrics && memoryMetrics.agents.length > 0 && (
          <section className="mb-6">
            <div className="flex items-center justify-between mb-3">
              ...
              {memoryMetrics.system && <SystemMemoryIndicator system={memoryMetrics.system} />}
            </div>
            <div className="bg-bg-secondary border border-border rounded-lg p-6">
  ...
  return (
    <div className="text-center">
      ...
      <div className="h-1 bg-border rounded-full mt-3 overflow-hidden">
        <div
          className="h-full bg-gradient-to-r from-accent to-accent-purple rounded-full transition-all duration-500"
